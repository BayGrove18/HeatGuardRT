...
### Q19：任务优先级是如何划分的？为什么升级任务最低？

答：优先级按安全时效和是否可重传划分。`control_task` 为 6，负责状态机、故障和安全交接，必须最先运行；按键任务为 5 和 4，保证交互事件尽快进入状态机；传感器为 3，监督为 2，升级为 1。升级数据可以通过协议重传，Flash 操作也较长，不应压制门锁、故障和控制路径。实际优先级不是“数字越大越重要”的口号，而是结合任务阻塞关系、最坏执行时间和安全后果安排。

### Q20：任务栈深度怎样估算和验证？栈溢出时怎么办？

答：当前创建任务时显式给出 128 到 256 words 的栈深；包含帧解析器局部数组的 `upgrade_task` 给 256 words，控制任务同样给 256 words。初始值来自调用链和局部对象估算，但不能只凭经验。运行时通过 `uxTaskGetStackHighWaterMark()` 记录控制任务的最小余量，实板应在最坏场景下检查所有任务。若触发 `vApplicationStackOverflowHook`，代码先关断执行器，再禁中断等待 IWDG 复位。

### Q21：为什么选择 `heap_4`？动态创建任务失败如何处理？

答：项目使用 FreeRTOS `heap_4`，它支持释放后相邻空闲块合并，适合启动阶段动态创建任务、队列和 mutex 的场景。这里对象都在启动时创建，运行期没有频繁分配释放，碎片风险较低。每个 `xTaskCreate()` 和 `xQueueCreate()` 后都用 `configASSERT` 检查；若内存不足，`vApplicationMallocFailedHook` 会先执行安全关断，不会让设备带着不完整任务集继续运行。

### Q22：为什么传感器任务使用 `vTaskDelayUntil()`，按键任务使用 `vTaskDelay()`？

答：DHT11 是固定 1 s 周期采样，`vTaskDelayUntil()` 以绝对节拍为基准，单次读取耗时波动不会不断累积成周期漂移。按键消抖和长按采样只关心“从当前动作开始再等待 20 ms 或 10 ms”，属于相对延时，用 `vTaskDelay()` 更直接。两者都让出 CPU；不能在任务中用空循环延时，否则会破坏实时调度和 Tickless 空闲判断。

### Q23：TIM2、TIM3、TIM4 在项目中各自承担什么？为什么倒计时独立用 TIM4？

答：TIM2 的 CH2 和 CH3 分别驱动门舵机和加热 PWM，TIM3 CH3 驱动转盘 PWM，TIM4 仅产生倒计时更新中断。同一个定时器的通道共享 PSC 和 ARR，如果把不同频率的倒计时中断和舵机 PWM 混在同一时基中，会互相影响。旧版项目曾遇到这类冲突，因此当前将倒计时移到 TIM4，安全关断时同时禁用 TIM4 更新中断和计数器，职责更清晰。

### Q24：加热过程中收到升级请求，系统如何避免带载复位？

答：升级任务完成整镜像校验和 Manifest 发布后，不直接调用复位，而是通过回调投递 `CONTROL_EVENT_UPDATE_REQUEST`。控制状态机收到后先将 `heater_enabled` 清零并进入 `UPDATE_PENDING`；`control_task` 在本轮完成输出发布后才调用 `upgrade_commit_after_safe_state()` 写 BKP 邮箱并复位。这个顺序保证软件交接前已经走过统一的执行器安全边界，而不是让低优先级升级任务绕过控制路径。

### Q25：W25Q64 写入有哪些边界条件？代码如何防止 Flash 操作卡死或越页？

答：W25Q64 一次页编程最多 256 B，跨页写可能回卷覆盖当前页开头。`w25q64_write()` 根据当前地址剩余页空间拆分数据，每个块都不跨页；读写先检查地址范围，擦除范围按 4 KiB 扇区向下、向上对齐。写或擦除后轮询状态寄存器 BUSY 位，并用 FreeRTOS tick 设置 500 ms 超时；超时就向上返回失败，由升级会话中止，避免无限等待。

### Q26：DMA 环形缓冲区如何处理半传输、全传输和 IDLE 同时到来？

答：代码维护 `ring_read_index`，DMA 写指针由当前剩余传输计数换算得到。半传输时排空 `[read, 128)`，全传输时排空 `[read, 0)` 并处理回绕；IDLE 时按实时 DMA 计数排空尚未到半缓冲区的尾部。每次排空都只移动读指针到实际写指针，重复触发不会重复投递数据。若字节队列已满，设置 `transport_overflow` 并丢弃当前未排空段，升级任务随后中止会话和复位解析器。

### Q27：为什么 DMA/USART 中断优先级设为 3？如果再调高会怎样？

答：当前 DMA 和 USART ISR 内调用 `xQueueSendFromISR()`、`portYIELD_FROM_ISR()`，所以它们必须处于 FreeRTOS 允许调用 API 的优先级范围。工程把 `configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY` 设为 3；在 Cortex-M 中数值越小优先级越高，因此 3 是允许调用 RTOS API 的最高紧急级别。若改成 0、1、2 仍调用这些 API，内核临界区可能被非法打断，配置断言或运行时行为都会出问题。

### Q28：进入 STOP 前后为什么要恢复系统时钟？只重新打开 SysTick 不行吗？

答：STM32F1 从 STOP 唤醒后系统时钟回到低速内部时钟，PLL/HSE 不会自动恢复到运行前配置。SysTick 的重装值、TIM1 微秒延时、UART 波特率和外设总线时钟都依赖系统频率；只重开 SysTick 会导致 tick 和外设时序错误。因此 `power_manager` 唤醒后先调用 `SystemClock_RestoreAfterStop()`，再按 `SystemCoreClock` 重装 SysTick，并最后才允许调度继续执行。

### Q29：PA0 升级唤醒线为什么必要？直接依赖 USART RX 唤醒不可以吗？

答：STM32F103 处于 STOP 时主时钟停止，不能假设 UART 接收到第一字节后仍能可靠地完成完整接收和 DMA 搬运。项目用 PA0 的 EXTI 作为独立低电平唤醒信号，主机先拉低 PA0，设备退出 STOP 并恢复时钟，再开始 UART 升级传输。它不是传输协议的一部分，而是电源状态和通信状态之间的硬件握手；没有这根线就应禁用待机阶段 STOP 或设计上位机预唤醒窗口。

### Q30：你如何评价当前项目的代码质量，下一步最值得补什么？

答：已具备单写者状态机、执行器边界、错误优先安全关断、编译零警告、宿主机单测和 CI，但测试主要覆盖纯 C 逻辑，外设时序还未闭环。下一步优先补实板故障注入和可观测性：用示波器记录门锁到 PWM 归零时延，用调试器采集各任务栈水位，用故障桩模拟 DMA 队列满和 W25Q64 BUSY 超时，用电流表记录 STOP 电流和 RTC 漂移。先得到量化证据，再把结果写进简历。
